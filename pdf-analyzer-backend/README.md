...
![Backend Architecture](https://raw.githubusercontent.com/THILLAINATARAJAN-B/pdf-analyzer/assets/Images/backend_architecture_diagram.png)
---
...
- **Manual redirect following** with a hard cap of 3 hops — prevents open redirect exploitation and infinite redirect loops
...
- **Primary:** Google Gemini 1.5 Flash — structured JSON output enforced via schema prompt
...
| OCR Engine | Tess4J 5.11 (Tesseract LSTM wrapper) |
...
| `422` | Too many redirects | `Too many redirects (max 3).` |
...
| Too many redirects (>3) | `PdfDownloadService` manual redirect handler | `422` — redirect loop protection |
...
The Dockerfile uses `eclipse-temurin:17-jre-alpine` as the runtime base and installs Tesseract via `apk`. API keys are injected at runtime and never baked into the image.
...
# OCR
pdf.ocr.tessdata-path=/usr/share/tessdata
pdf.ocr.max-pages=10
